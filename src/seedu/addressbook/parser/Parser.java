...
public class Parser {
    ...
    public static final Pattern PERSON_DATA_ARGS_FORMAT = // '/' forward slashes
                                                          ...
                                                          // delimiter prefixes
    Pattern.compile(
            "(?<name>[^/]+)" + " (?<isPhonePrivate>p?)p/(?<phone>[^/]+)" + " (?<isEmailPrivate>p?)e/(?<email>[^/]+)"
                    + " (?<isAddressPrivate>p?)a/(?<address>[^/]+)" + "(?<tagArguments>(?: t/[^/]+)*)"); // variable
                                                                                                         // number
                                                                                                         // of
                                                                                                         // tags
    ...
    public Command parseCommand(String userInput) {
        ...
        case EditCommand.COMMAND_WORD: // new command
            return prepareEditCommand(arguments);

        case EditCommand.NEXT_COMMAND_WORD:
            return prepareAdd(arguments);
